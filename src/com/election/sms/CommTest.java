...
public class CommTest{
	private static String port, model;
	...
	static int bauds[] = { 9600, 14400, 19200, 28800, 33600, 38400, 56000, 57600, 115200, 230400, 460800, 921600 };
		...
		while (portList.hasMoreElements())
		{
			...
			if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL)
			{
				...
				for (int i = 0; i < bauds.length; i++)
				{
					...
					System.out.println("Printing bauds "+bauds[i]);
					try
					{
						...
						if (response.indexOf("OK") >= 0)
						{
							try
							{
								...
								//Model extracted
								model = response.replaceAll("\\s+OK\\s+", "").replaceAll("\n", "").replaceAll("\r", "");
								...
								//Start SMS listening
								SMSLibBeta smsLib = new SMSLibBeta("modem"+port, port, model, baud);
								smsLib.load();
